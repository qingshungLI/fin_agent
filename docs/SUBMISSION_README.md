...
当前为源码交付。完整研究需要用户提供授权数据和模型凭据，安装依赖不会自动获得这些资源。源码包也包含技术报告的 LaTeX 源文件、配图和去敏的截图证据，可独立进行合成数据工程测试；完整市场研究、独立确认和交易验证有额外数据与配置前提。

## 从代码包开始

将 code.zip 解压到本提交目录，得到同级 code/ 目录；公开介绍中的链接以此布局为准。进入 code 目录，推荐 Python 3.11。
...
技术报告和演示中的待执行消融方案不计为已完成实验。封面与正文统一使用探索性证据口径，独立确认尚未完成。
