...
\usepackage{graphicx,xcolor,enumitem,hyperref,listings,fancyhdr,tikz}
...
\Large 面向递归自我改进的可审计 Alpha 研究系统技术报告}
\author{AutoAlpha Research Team}
\date{2026 年 9 月 8 日}

\begin{document}
\maketitle
\begin{center}
\large\textcolor{auroragold}{\textbf{AURORA = Auditable Unified Recursive Optimization and Research Architecture}}\\
\normalsize AURORA 中的 RSI 指 Recursive Self-Improvement，而不是 Relative Strength Index。
\end{center}
\vspace{0.4em}
\begin{abstract}
AURORA Alpha Harness 是一个将大语言模型、量化因子研究、统计验证和实验审计组合起来的研究基础设施。系统的核心对象不是单个收益率最高的表达式，而是一个可复现的研究过程：AI 提出机制假设，受限 DSL 将假设编译为可执行因子，冻结数据切分、成本和断言，运行测量与证伪检验，再根据结构化证据生成下一代研究任务。每一次迭代都绑定父结构、预算、数据哈希、代码哈希和结果状态，因此研究方法可以递归改进，同时不会获得修改评估器、读取未来数据或把未知结果伪装成通过的权限。

本文描述当前仓库中的完整架构，包括数据平面、结构目录、因子 DSL、研究流水线、honest R-learning 浅森林、自动演化队列、DeepSeek 角色防火墙、审计与恢复、组合 gate、企业 SDK 及 3D 控制台。报告也区分正式配置与 fast 配置：fast 用于快速验证 RSI 工程闭环，不具有正式确认资格。
\end{abstract}
...
box/.style={draw=aurorablue!80,fill=blue!4,rounded corners,align=center,minimum width=3.0cm,minimum height=0.85cm},
feedback/.style={-{Latex[length=2.5mm]},thick,auroragold}, flow/.style={-{Latex[length=2.5mm]},thick,aurorablue}]
\node[box] (data) {????? MarketPanel};
\node[box,right=of data] (spec) {Structure??????????};
\node[box,right=of spec] (compile) {DSL ??????????};
\node[box,below=of compile] (measure) {???????IC?placebo??????};
\node[box,left=of measure] (forest) {?????honest ???};
\node[box,right=of measure] (audit) {??????SQLite ??};
\node[box,below=of forest] (evolve) {RSI ?????parent/operator/depth};
\node[box,below=of audit] (product) {SDK?API??????????};
\draw[flow] (data)--(spec);
\draw[flow] (spec)--(compile);
\draw[flow] (compile)--(measure);
\draw[flow] (measure)--(forest);
\draw[flow] (measure)--(audit);
\draw[feedback] (forest)--(evolve);
\draw[feedback] (audit)--(evolve);
\draw[feedback] (evolve)--(spec);
\draw[flow] (audit)--(product);
\draw[flow] (evolve)--(product);
\end{tikzpicture}
...
\code{engine/llm.py} & DeepSeek proposer、reviewer、bettor、reconciler、inducer 的角色隔离与缓存。 \\
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[node distance=8mm and 5mm, every node/.style={font=\small},
rstep/.style={draw=aurorablue!80,fill=blue!4,rounded corners,align=center,minimum width=2.45cm,minimum height=0.72cm}, arr/.style={-{Latex[length=2.2mm]},thick,aurorablue}]
\node[rstep] (p) {PROPOSE\\DeepSeek / 用户};
\node[rstep,right=of p] (c) {COMPILE\\受限 DSL};
\node[rstep,right=of c] (m) {MEASURE\\A 段统计};
\node[rstep,right=of m] (cr) {CRITICIZE\\四把刀};
\node[rstep,below=of cr] (r) {RECORD\\哈希与检查点};
\node[rstep,left=of r] (mu) {MUTATE\\受控子代};
\draw[arr] (p)--(c);\draw[arr] (c)--(m);\draw[arr] (m)--(cr);\draw[arr] (cr)--(r);\draw[arr] (r)--(mu);\draw[arr] (mu)--(p);
\end{tikzpicture}
...
森林只在冻结切点上生成浅树。每棵树使用独立日期块和样本权重，叶子必须满足独立日期和证券覆盖要求。森林输出变量选择频率和叶子诊断，不向 LLM 直接提供“因果效应方向”。
...
在同一台机器的全量冻结结构上，候选筛查加发现阶段从 454.923 秒降至 238.610 秒，约 1.907 倍加速；两折 p 值和候选频率逐值一致。该数字是阶段级同机复核，不是对所有机器的固定承诺。

\section{RSI 自动演化闭环}
\subsection{预算与队列}
\code{--full-grid} 当前建立 70 个初始 seed 任务，并预留 140 次自动演化预算，总尝试上限为 210。140 不是预先写死的 140 个表达式，而是后续队列可以消耗的预算；每完成一个结构，系统最多追加一个直接 follow-up，实际数量取决于可验证证据和可用格子。
...
SDK 的输出状态是 \texttt{NEEDS\_REVIEW} 或 \texttt{REPLICATION\_CANDIDATE}，不会把单次用户运行直接包装成正式金融结论。
...
\section{当前实现状态与已知边界}
截至 2026 年 9 月 8 日，仓库已经实现：
\begin{itemize}[leftmargin=2em]
  \item 70 格初始网格、140 次演化预算和最大深度 2 的自动演化队列；
  \item DeepSeek 五角色研究编排与响应缓存；
  \item A 段测量、placebo、断言、稳定性、增量、成本和条件发现；
  \item Windows 多进程森林，阶段级复核约 1.907 倍加速且统计输出一致；
...
# 3. 启动控制面板
.venv-research/Scripts/python.exe dashboard_server.py
\end{lstlisting}
