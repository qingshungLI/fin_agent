...
test('real 3D scene and map selection show the same research coordinate', async ({ page }) => {
  ...
  await expect(page.locator('.c-code')).toBeVisible();
  ...
  for (const [x, y] of [[.53, .6], [.48, .62], [.56, .56], [.45, .55]]) {
...
test('evidence and logs show actual artifacts with downloadable report', async ({ page }) => {
  ...
  await expect(page.locator('.c-detail pre')).toHaveCount(3);
...
test('composition panel shows actual rules, failures and downloadable frozen targets', async ({ page }) => {
  await openExistingRun(page);
  await expect(page.locator('.c-kpis')).toBeVisible();
...
test('fast profile displays actual reduced configuration without formal success', async ({ page }) => {
  await page.goto(base);
  await expect(page.locator('.c-kpis')).toBeVisible();
  await page.getByLabel('当前研究批次').selectOption('fastcycle-v2-20260908');
  await expect(page.locator('.c-alert').filter({ hasText: 'FAST 探索' })).toContainText('19 次横截面诊断');
  await expect(page.locator('.c-alert').filter({ hasText: 'FAST 探索' })).toContainText('不授予正式 PASS');
  await page.getByRole('button', { name: '研究日志', exact: true }).click();
